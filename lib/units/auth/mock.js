    ...
    switch (req.accepts(['json'])) {
      case 'json':
        requtil.validate(req, function() {
          ...
          log.info('registe user is ' + req.body.name + ' passwd is ' + req.body.passwd)
          if(req.body.passwd !== req.body.repasswd) {
            throw new requtil.ValidationError('两次密码不一致', req.validationErrors())
          }
        })
          .then(function() {
            log.info('registe "%s"', req.body.email)
            dbapi.registeUser({
              name: req.body.name
    ...
    switch (req.accepts(['json'])) {
      case 'json':
        requtil.validate(req, function() {
            ...
            req.checkBody('email').isEmail()
            req.checkBody('passwd').notEmpty()
            log.info('login user is ' + req.body.name + ' passwd is ' + req.body.passwd)
            if(req.body.name !== 'xiangleilei' && req.body.passwd !== '123456') {
